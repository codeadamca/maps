...
if (!$connect) respond(false, null, "Database connection failed");
...
if ($method === 'DELETE' && preg_match('#^/design/([^/]+)$#', $path, $m)) delete_design($connect, $m[1]);
...
function create_owner($connect) {
    ...
    $owner_id = $data['owner_id'] ?? "OWNER-" . strtoupper(bin2hex(random_bytes(4)));
...
function create_design($connect) {
    ...
    // AUTO CREATE OWNER IF NONE PROVIDED
    if (!$owner_id) {
        $owner_id = "OWNER-" . strtoupper(bin2hex(random_bytes(4)));
        mysqli_query($connect, "INSERT INTO owners (owner_id) VALUES ('$owner_id')");
    }
